...
class PcComponentesScraper {
    ...
    /**
     * Extraer información completa del producto de PcComponentes
     */
    public function extractProductInfo() {
        $startTime = microtime(true);

        try {
            // Primero visitar la home para establecer cookies (simula navegación real)
            $this->fetchHTML('https://www.pccomponentes.com/');

            // Pausa LARGA y aleatoria entre 5-15 segundos (imita usuario real leyendo la home)
            // Esto es CRÍTICO para evitar detección de bots
            $delay = rand(5, 15);
            error_log("Esperando {$delay} segundos antes de acceder al producto (anti-bot)");
            sleep($delay);

            // Ahora obtener la página del producto
    ...
    private function fetchHTML($url) {
        // IMPORTANTE: NO randomizar User-Agent
        // Usar UN SOLO User-Agent consistente (Chrome en Windows es el más común)
        ...
        $options = [
            ...
            CURLOPT_REFERER => 'https://www.pccomponentes.com/',
            CURLOPT_COOKIEJAR => sys_get_temp_dir() . '/pccomp_' . md5($this->url) . '.txt',
            CURLOPT_COOKIEFILE => sys_get_temp_dir() . '/pccomp_' . md5($this->url) . '.txt',
            CURLOPT_HTTPHEADER => [
    ...
    private function extractTitle() {
        return $this->extractByXPath('//*[@id="pdp-title"]');
    }

    /**
     * Extraer precio del producto
     * XPath: //*[@id="pdp-price-current-integer"]
     */
    private function extractPrice() {
        $priceText = $this->extractByXPath('//*[@id="pdp-price-current-integer"]');
     ...
     * XPath: /html/body/div[2]/main/div[2]/div[3]/div[2]/div[2]/div[1]/div/ul/li[1]/img
     */
    private function extractImage() {
        ...
        $imagePaths = [
            '/html/body/div[2]/main/div[2]/div[3]/div[2]/div[2]/div[1]/div/ul/li[1]/img',
            '//img[@id="pdp-img-zoom"]',
            '//div[contains(@class, "product-image")]//img',
            '//div[contains(@class, "gallery")]//img[1]',
            ...
            if ($nodes->length > 0) {
                ...
                if ($src && (strpos($src, '.jpg') !== false || strpos($src, '.png') !== false || strpos($src, '.webp') !== false)) {
                    ...
                    if (strpos($src, 'http') !== 0) {
                        $src = 'https://www.pccomponentes.com' . $src;
                    }
                    return $src;
                }
            }
    ...
    /**
     * Extraer descuento
     * XPath: //*[@id="pdp-price-discount"]
     */
    private function extractDiscount() {
        $discountText = $this->extractByXPath('//*[@id="pdp-price-discount"]');
    ...
    /**
     * Método estático para verificar si una URL es de PcComponentes
     */
    public static function isPcComponentesURL($url) {
        return strpos($url, 'pccomponentes.com') !== false;
    }
