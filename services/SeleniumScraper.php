...
class SeleniumScraper {
    ...
    /**
     * Extraer información del producto usando Hero
     */
    public function extractProductInfo() {
        $startTime = microtime(true);

        try {
            // Usar Hero scraper (Node.js)
            $heroScript = __DIR__ . '/../scrapers/hero_scraper.js';

            if (!file_exists($heroScript)) {
                return ['success' => false, 'error' => 'Script de Hero no encontrado'];
            }

            // Ejecutar script Node.js Hero
            $command = sprintf(
                'cd %s && node %s %s 2>&1',
            ...
            // La primera línea con JSON es el resultado
            $jsonOutput = null;
            ...
            if (!$jsonOutput) {
                $errorMsg = implode("\n", $output);
                error_log("Hero scraper error: $errorMsg");
            ...
            if (!$result) {
                error_log("Invalid JSON from Hero: $jsonOutput");
