...
class HeroScraper {
    ...
    async scrape() {
        try {
            ...
            await this.hero.goto(this.url, { timeoutMs: 45000 });

            // Esperar a que la página cargue completamente
            console.error('⏳ Esperando carga completa...');
            await this.hero.waitForPaintingStable({ timeoutMs: 30000 });
    ...
    async extractDecathlon() {
        ...
        try {
            ...
            // Precio actual
            try {
                const priceElem = await this.hero.document.querySelector('span.notranslate.vp-price-amount.vp-price-amount--large[data-part="amount"]');
                if (priceElem) {
                    const priceText = await priceElem.textContent;
                    if (priceText) {
                        this.result.price = this.parsePrice(priceText);
            ...
            // Calcular descuento basado en precio actual y precio anterior
            if (this.result.price && this.result.original_price && this.result.original_price > this.result.price) {
                this.result.discount = Math.round(((this.result.original_price - this.result.price) / this.result.original_price) * 100);
                console.error('✓ Descuento calculado:', this.result.discount + '%');
            }
    ...
    async extractIkea() {
        ...
        try {
            ...
            // Imagen del producto - Buscar la primera imagen con clase pipf-image
            try {
                const imgElem = await this.hero.document.querySelector('img.pipf-image');
                if (imgElem) {
                    const src = await imgElem.src;
                    if (src && src.includes('ikea.com')) {
                        this.result.image = src;
                        console.error('✓ Imagen encontrada (pipf-image)');
                    }
                }

                // Fallback a pip-image si no se encuentra pipf-image
                if (!this.result.image) {
                    const imgElemFallback = await this.hero.document.querySelector('img.pip-image');
                    if (imgElemFallback) {
                        const src = await imgElemFallback.src;
                        if (src && src.includes('ikea.com')) {
                            this.result.image = src;
                            console.error('✓ Imagen encontrada (pip-image)');
                        }
                    }
                }
            } catch (e) {
